...
namespace tractor {
    namespace comms {
        ...
        Serial::Serial() : pimpl_(std::make_unique<Impl>()) {}
        ...
        Serial::Serial(const std::string &port, const SerialConfig &config) : pimpl_(std::make_unique<Impl>()) {
            pimpl_->port = port;
        ...
        bool Serial::is_open() const { return pimpl_->fd >= 0; }
        ...
        // Write operations
        ssize_t Serial::write(const uint8_t *data, size_t size) {
        ...
        ssize_t Serial::write(const std::vector<uint8_t> &data) { return write(data.data(), data.size()); }
        ...
        std::vector<uint8_t> Serial::read_until(uint8_t delimiter, size_t max_size) {
        ...
        std::string Serial::read_line(size_t max_size) {
        ...
        ssize_t Serial::read_exact(uint8_t *buffer, size_t size, uint32_t timeout_ms) {
            if (buffer == nullptr || size == 0) {
                return 0;
            }
        ...
        std::vector<uint8_t> Serial::read_exact(size_t size, uint32_t timeout_ms) {
        ...
        // Buffer operations
        ssize_t Serial::available() const {
            if (!is_open()) {
                return -1;
            }
        ...
        bool Serial::set_baud_rate(uint32_t baud_rate) {
            if (!is_open()) {
                pimpl_->config.baud_rate = baud_rate;
                return true;
            }
        ...
        bool Serial::set_termios_baud(uint32_t baud_rate) {
            struct termios tty;
            if (tcgetattr(pimpl_->fd, &tty) != 0) {
                pimpl_->last_error = std::string("Failed to get attributes: ") + std::strerror(errno);
                return false;
        ...
        uint32_t Serial::get_baud_rate() const {
            if (!is_open()) {
        ...
        void Serial::set_read_timeout(uint32_t timeout_ms) { pimpl_->config.read_timeout_ms = timeout_ms; }

        void Serial::set_write_timeout(uint32_t timeout_ms) { pimpl_->config.write_timeout_ms = timeout_ms; }

        bool Serial::set_data_bits(DataBits data_bits) {
            pimpl_->config.data_bits = data_bits;
            return is_open() ? configure_port() : true;
        }

        bool Serial::set_parity(Parity parity) {
            pimpl_->config.parity = parity;
            return is_open() ? configure_port() : true;
        }

        bool Serial::set_stop_bits(StopBits stop_bits) {
            pimpl_->config.stop_bits = stop_bits;
            return is_open() ? configure_port() : true;
        }

        bool Serial::set_flow_control(FlowControl flow_control) {
            pimpl_->config.flow_control = flow_control;
            return is_open() ? configure_port() : true;
        }

        bool Serial::apply_config(const SerialConfig &config) {
            pimpl_->config = config;
            return is_open() ? configure_port() : true;
        }

        SerialConfig Serial::get_config() const { return pimpl_->config; }

        std::string Serial::get_port() const { return pimpl_->port; }

        std::string Serial::get_last_error() const { return pimpl_->last_error; }
        ...
        bool Serial::port_exists(const std::string &port) {
            try {
        ...
        bool Serial::set_dtr(bool state) {
            if (!is_open()) {
                pimpl_->last_error = "Port is not open";
                return false;
            }
        ...
        bool Serial::set_rts(bool state) {
            if (!is_open()) {
                pimpl_->last_error = "Port is not open";
                return false;
            }
        ...
        void Serial::send_break(uint32_t duration_ms) {
            if (!is_open()) {
                return;
            }
        ...
        int Serial::get_fd() const { return pimpl_->fd; }
